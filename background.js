...
const wrestlingManualBans = [
    ...
    '/wrestlers/nikita-naridian', '/wrestlers/riho', '/wrestlers/thekla', '/wrestlers/pj-vasa',
    ...
    try {
        ...
        const data = await new Promise(resolve => {
            if (typeof browser !== 'undefined') {
                localApi.get([WRESTLING_CACHE_TIME_KEY, 'wrestling_women_urls', 'v27_chrome_bump', WRESTLING_CACHE_BUMP_KEY]).then(resolve);
            } else {
                localApi.get([WRESTLING_CACHE_TIME_KEY, 'wrestling_women_urls', 'v27_chrome_bump', WRESTLING_CACHE_BUMP_KEY], resolve);
            }
        });
        ...
        if (!data.v27_chrome_bump) {
            ...
            await new Promise(resolve => {
                if (typeof browser !== 'undefined') {
                    localApi.set({ 'v27_chrome_bump': true, [WRESTLING_CACHE_BUMP_KEY]: true }).then(resolve);
                } else {
                    localApi.set({ 'v27_chrome_bump': true, [WRESTLING_CACHE_BUMP_KEY]: true }, resolve);
                }
            });
        }
        else if (!data[WRESTLING_CACHE_BUMP_KEY]) {
            console.log('Force-refreshing wrestling roster cache via V38 dynamic-name bump...');
            await new Promise(resolve => {
                if (typeof browser !== 'undefined') {
                    localApi.remove([WRESTLING_CACHE_TIME_KEY, 'wrestling_women_urls']).then(resolve);
                } else {
            ...
            await new Promise(resolve => {
                if (typeof browser !== 'undefined') {
                    localApi.set({ [WRESTLING_CACHE_BUMP_KEY]: true }).then(resolve);
                } else {
                    localApi.set({ [WRESTLING_CACHE_BUMP_KEY]: true }, resolve);
                }
            });
        } 
