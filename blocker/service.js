...
// Firefox Enhancer uses manifest incognito=spanning. Apply the standalone
// Focus Master rule that the master Enabled switch controls normal windows only:
// private tabs remain enforced even when the normal-window toggle is off.
function getEffectiveEnforcementSettings(settings, incognito = false) {
...
async function prepaintTimeRuleCheck(message, sender) {
  ...
  const settings = getEffectiveEnforcementSettings(storedSettings, Boolean(sender?.tab?.incognito));
...
async function evaluateNavigation(tabId, url, title = '') {
  ...
  const [dataset, storedSettings, tabInfo] = await Promise.all([
    loadDataset(),
    getSettings(),
    browser.tabs.get(tabId).catch(() => null)
  ]);
  const settings = getEffectiveEnforcementSettings(storedSettings, Boolean(tabInfo?.incognito));
...
browser.tabs.onUpdated.addListener((tabId, changeInfo, tab) => {
  ...
  // Never pair a freshly changed URL with tab.title: Firefox can briefly keep
  // the previous page title during navigation. webNavigation checks the URL,
  // then the destination title is checked when Firefox reports its title update.
...
void getSettings()
  .then(settings => refreshTimeRulePrepaintRegistration(settings))
  .catch(error => console.warn('[BraveFox Focus Master] Initial Firefox Time Rule pre-paint registration failed:', error));

initializeTimeRuleTracking({
  getContext: async () => {
    const [settings, dataset] = await Promise.all([getSettings(), loadDataset()]);
    return { settings, profile: dataset.profile };
  },
