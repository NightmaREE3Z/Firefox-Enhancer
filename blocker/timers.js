...
const TRACKER_CONTEXT = globalThis.chrome?.extension?.inIncognitoContext ? 'incognito' : 'firefox-spanning';
...
async function runFocusedUsageSample() {
  ...
  // Firefox Enhancer uses a spanning private-browsing background rather than
  // Chromium's split Incognito worker. Keep the Chrome behavior: disabling the
  // normal-window master switch must not disable enforcement/time accounting in
  // a private tab. Usage still lands in the same per-profile quota bucket.
  const effectiveSettings = tab?.incognito && context?.settings
    ? { ...context.settings, enabled: true }
    : context?.settings;

  if (tab?.url && !isTwitchUrl(tab.url) && effectiveSettings?.enabled) {
    currentRules = profileRules(effectiveSettings, 'quotaRules', context.profile)
