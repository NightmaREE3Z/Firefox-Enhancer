...
(() => {
	...
	"Vice City", "Liberty City", "San Andreas", "North Yankton", "Yankton", "Rockstar", "North", "South", "West", "East", 
    ]; 
    ...
    const handleRedirects = () => {
        try {
            ...
            if (!isBlocked) {
                const rawPathSearch = decodeURIComponent(urlObj.pathname + urlObj.search).toLowerCase();
    ...
    const interceptNavigation = () => {
        try {
            ...
            const submitHandler = (event) => {
                const form = event.target; const action = form.action || '';
                if (blockedUrls.some(blockedUrl => blockedUrl.test(action))) {
                    event.preventDefault(); event.stopPropagation();
                }
            };
            onWindowEvent(document, 'click', clickHandler, true); onWindowEvent(document, 'submit', submitHandler, true);
        } catch (e) {}
    };
    ...
    const ensureDOMReady = () => {
        if (document.readyState === 'loading') {
            ...
            observeDOMChanges(); observeForRestrictedPhrases(); interceptNavigation(); hookHistoryAPI();
        }
    };
