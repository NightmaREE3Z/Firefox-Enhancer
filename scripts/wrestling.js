...
(function () {
    ...
    // === MANUAL SAFETY NET ===
    // If someone is missing from the main roster page (like DLC or NXT rookies), add their slug here!
    const manualBans = [
    ...
    // --- UNIVERSAL STORAGE WRAPPER FOR CHROME & FIREFOX ANDROID ---
    function getExtensionStorage() {
        if (typeof browser !== 'undefined' && browser.storage && browser.storage.local) {
            return browser.storage.local; // Firefox Native
        } else if (typeof chrome !== 'undefined' && chrome.storage && chrome.storage.local) {
            return chrome.storage.local; // Chrome Native / Polyfill
        }
        return null;
    }

    // Function to cleanly broadcast only safe URLs to the rest of the browser extensions
    function broadcastToExtensions(urls) {
        const storageAPI = getExtensionStorage();
        if (storageAPI) {
            const safeUrls = urls.filter(url => {
                const slug = url.toLowerCase();
                return !doNotBroadcast.some(blocked => slug.includes(blocked));
            });
            storageAPI.set({'wrestling_women_urls': safeUrls}, () => {
                // Checking runtime error specifically based on the API used
                ...
                if (err) {
                    devLog('Error broadcasting to storage: ' + err.message);
                } else {
                    devLog(`Successfully broadcasted ${safeUrls.length} safe URLs to cross-browser storage.`);
                }
            });
        } else {
            devLog('WARNING: No cross-extension storage API found in this context (sandbox restriction).');
        }
    }
    ...
    function hideUIElements() {
        ...
        `;
        // Safely append to head if it exists, otherwise the root html element
    ...
    function getCachedWomenUrls() {
        ...
        if (cached) {
            try {
                ...
                urls = [...new Set([...urls, ...parsed])]; // Merge and remove duplicates
            } catch (e) {
        ...
        try {
            ...
            dls.forEach(dl => {
                ...
                if (text.includes('Known as:')) {
                    ...
                    if (afterKnown) {
                        // Extract until the end of the line
    ...
    function flushDiscoveredAliases(currentBlockedUrls) {
        ...
        if (added) {
            ...
            broadcastToExtensions(updatedUrls); // Use the safe broadcasting wrapper
            devLog(`Learned and broadcasted new aliases! Global list is now: ${updatedUrls.length} profiles.`);
        }
        
        newlyDiscovered = []; // reset for next pass
        return updatedUrls;
    }

    // Helper: Chunks an array into smaller arrays to prevent browser throttling on fetch
    ...
    // Function to fetch and update the women URLs across multiple database pages
    async function updateWomenUrls() {
        const cachedTime = localStorage.getItem(CACHE_TIME_KEY);
        const now = Date.now();

        // Broadcast the current cache immediately so scripts listening (Instagram) get data instantly
        const currentCache = getCachedWomenUrls();
        broadcastToExtensions(currentCache);

        // Only fetch if cache is missing or older than 12 hours
        if (cachedTime && (now - parseInt(cachedTime, 10)) < CACHE_LIFETIME_MS) {
            devLog('Using recently cached list of women profiles.');
            return currentCache;
        }

        devLog('Fetching roster and database pages to build comprehensive list...');
        
        // Multi-page fetch array: WWE Pages + Database pages + All-Time Rosters + Hall of Fame
        const pagesToFetch = [
        ...
        try {
            // Process requests in batches of 4 to prevent network timeouts and browser throttling
            const chunks = chunkArray(pagesToFetch, 4);
                ...
                const fetchPromises = chunk.map(async (url) => {
                    try {
                        ...
                        if (url.includes('/roster/')) {
                            ...
                            // Extract all links from the female search results pages
            ...
            combinedUrls = [...new Set(combinedUrls)]; // Remove duplicates

            if (combinedUrls.length > 0) {
                localStorage.setItem(CACHE_KEY, JSON.stringify(combinedUrls));
                localStorage.setItem(CACHE_TIME_KEY, now.toString());
                
                // BROADCAST THE FRESHLY FETCHED URLS TO CHROME EXTENSION STORAGE
            ...
            return getCachedWomenUrls(); // Fallback to whatever we have if offline
        }
    }
    ...
    function obliterateBlockedElements(blockedUrls) {
        ...
        // AGGRESSIVE CONTEXT SCANNER: If we are actively on the female database page, NUKE EVERYTHING
        ...
        // Standard targeted scanner for all other pages
        ...
        // 1. Target exact roster links first (Primary profiles & Game Rosters)
        ...
        // 2. Target search result containers, game rosters, table reigns, blog posts, images, and stray elements
    ...
    async function initFilter() {
        ...
        // 0. Force an immediate broadcast on load so dependent scripts get data instantly
        broadcastToExtensions(getCachedWomenUrls());

        // Force a fresh fetch to grab the expanded database pages if we haven't done it yet
        if (!localStorage.getItem('wrestling_cache_v2')) {
             localStorage.removeItem(CACHE_TIME_KEY); 
             localStorage.setItem('wrestling_cache_v2', 'true');
        }

        // 1. Immediately block current page if it exists in our current cache
        ...
        // 2. Clear current elements based on existing cache
        blockedUrls = obliterateBlockedElements(blockedUrls);

        // 3. Initiate a background update. If new URLs are found, re-run filtering!
        ...
        // 4. Set up an observer to catch any dynamically loaded profiles (infinite scroll, AJAX tabs, etc.)
        ...
        // Use documentElement instead of body so it works instantly at document_start
